...
let lastChampId = null; // Track champion changes during ChampSelect stage
...
(async () => {
    ...
    const getRecommendedRunepage = (recommendations, champId) => {
        const result = recommendations.find((champ) => champ.championId === champId);
        if (!result) {
            console.log("The champ you've selected doesn't seem to have recommended runes.");
            return;
        }
        ...
        console.log("[RunepageHandler/Get] Recommended runepage retrieved. Selecting...");
        return pagePayload;
    };
    const selectRecommendedRunepage = async (champId, champName, autoRunepage) => {
        const pagePayload = getRecommendedRunepage(RECOMMENDED_RUNES, champId);
        if (!pagePayload)
            return;
        try {
            console.log(`[RunepageHandler/Select] Unlisted champ detected: ${champName.toUpperCase()}`);
            console.log(`[RunepageHandler/Select] Editing ${autoRunepage.name}...`);
            // Apply the rune page
            await leagueRequest.put(`/lol-perks/v1/pages/${autoRunepage.id}`, pagePayload);
            return console.log(`[RunepageHandler/Select] Runepage ${autoRunepage.name} edited successfully!\n`);
        }
        catch (err) {
            if (isAxiosError(err)) {
                console.error("[Axios] Error setting a recommended runepage: ", err.response?.data || err.message);
            }
            else {
    ...
    const handleRunepageForRole = async (champName, champId, runePages) => {
        let matched = false;
        const categories = [
            CHAMPS.TANK_CHAMPS,
        ...
        for (const category of categories) {
            if (category.champList.includes(champName)) {
                matched = true;
                console.log(`[RunepageHandler] ${category.categoryName} detected: ${champName.toUpperCase()}`);
                const targetRunePage = runePages.find((p) => p.name === category.runePageName);
                if (!targetRunePage) {
                    console.warn(`[RunepageHandler] Runepage ${category.runePageName} not found`);
                    return;
                }
                const pagePayload = {
                ...
                if (!targetRunePage.current) {
                    ...
                    console.log(`[RunepageHandler] Applied rune page ${targetRunePage.name}\n`);
                }
                else {
                    console.log(`[RunepageHandler] The ${targetRunePage.name} rune page is already active.\n`);
                }
                break;
            }
        }
        if (!matched && AUTO_SELECT_RECOMMENDED_RUNES) {
            const autoRunepage = runePages.find((p) => p.name.toUpperCase() === "AUTORUNEPAGE");
            if (!autoRunepage) {
                return console.log("AutoRunepage not found.\nIf you'd like this feature, create a rune page named AutoRunepage");
            }
    ...
    const tryInviteFriends = async (partyMembers, isLobbyFull) => {
    ...
    const handleChampSelect = async (gameMode) => {
        try {
            if (!isInChampSelect) {
            ...
            const { data: session } = await leagueRequest.get("/lol-champ-select/v1/session");
            ...
            const { data: pages } = await leagueRequest.get("/lol-perks/v1/pages");
            ...
            if (isAxiosError(err)) {
                console.error("[Axios] ChampSelect error: ", err.response?.data || err.message);
            }
            else {
            ...
            switch (gamePhase) {
                case "ChampSelect":
                    if (AUTO_SELECT_RUNES)
                        await handleChampSelect(gameMode);
                    if (isGameAccepted)
