...
const champNamesConfigPath = path.join(
  rootDir,
  "config",
...
export const handleLeveling = async () => {
  ...
  try {
    ...
    let champName = "";
    if (!champNameFetched) champName = getChampName(allGameData);
    if (!champName) return;
    ...
    if (!isGamemodeFetched) GAMEMODE = fetchGamemode(gameInfo, champName);
    if (!GAMEMODE) return;
